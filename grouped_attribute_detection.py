...
DATASET_PATH = "."
ORIGINAL_TRAINING_DF = "attributes_training_df.csv"
ORIGINAL_TESTING_DF = "attributes_testing_df.csv"
...
def generate_file_name(name, type="--"):
    return name + "_" + type + "_" + time.strftime("%Y_%m_%d.%H%M%S")


def load_data_frame(df_path):
...
def create_new_model(base_model_name=None, train_base=False,
    ...
    # Adds 2 dense layers and corresponding drop-out layers
...
def load_training_data(group="Collar"):
    """
    Loads the dataframes, filters the groups of features and return the training df, testing df and the list of features
    :param feature_group:
    :return: training_df, testing_df, feature_list
    """
    additional_columns = ["filename", "Name", "Price"]
    global dataset_path
    if not dataset_path:
        dataset_path = DATASET_PATH
    training_df = load_data_frame(os.path.join(dataset_path, ORIGINAL_TRAINING_DF))
    testing_df = load_data_frame(os.path.join(dataset_path, ORIGINAL_TESTING_DF))
    features = list(training_df.columns)
    feaatures = set(features).difference(set(additional_columns))
    ...
    feature_group_names = sorted(list(feature_groups.keys()))
    logger.info(f"The different feature groups are {feature_group_names}")
    logger.info(f"Selecting the feature for this model: {group}")

    target_features = feature_groups.get(group)
    # target_features = additional_columns + target_features
    fname = f"features_{group}.pickle"
    with open(fname, "wb") as f:
        pickle.dump(target_features, f)
    logger.info(f"Saving the feature list as : {fname}")

    return training_df[additional_columns+target_features], testing_df[additional_columns+target_features], target_features
...
def main(use_class_weights=False):
    ...
    training_df, testing_df, label_columns = load_training_data()
    num_categories = len(label_columns)
    logger.info(f"The list of features for this model is : {label_columns}")
    logger.info(f"The number of features = {num_categories}")
    ...
    history_file_name = generate_file_name(model_name, type="history")+"pickle"
    with open(generate_file_name(model_name, type="history")+"pickle", "wb") as f:
        pickle.dump(history, f)
    logger.info(f"History saved to file: {history_file_name}")
...
if __name__ == '__main__':
    ...
    parser.add_argument("--dataset-path", default=None, required=False,
                        help="The directory where the training and testing data are located on the local server. This directory also contains the required " +
                        "data frames")
